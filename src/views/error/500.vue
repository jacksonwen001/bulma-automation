<template>
    <div class="body">
        <h1>500</h1>
        <div class="error-500" ref="error500"
            data-text="Oh no! Our code is not working properly. We will be back soon!">
        </div>
    </div>
</template>
...
const error500 = ref(null)

onMounted(() => {
    console.log(error500)

    let i = 0, data = "", text = error500.value.attributes["data-text"].value;
    let typing = setInterval(() => {
        if (i == text.length) {
            clearInterval(typing);
        } else {
            data += text[i];
            error500.value.attributes["data-text"].value = data;
            i++;
        }
    }, 100);
